...
# Load and preprocess the documents as you did before
...
@eel.expose
def perform_new_cosine(query, selected_documents):
    # Preprocess the query
    extras = ['!', '@', '#', '$', "%", "^", "&", "*", "(", ")", "<", ">", ",", ".", "[", "]", "{", "}", "_"]
    for char in extras:
        query = query.replace(char, '')

    # Create a list of all documents to be ranked
    documents_to_rank = []
    ...
    vectorizer.fit(retrieved_documents.values())
    query_vector = vectorizer.transform([query])

    # Calculate cosine similarity scores for the selected documents
    for name in selected_documents:
        score = 1.0  # Set the score to 1 for selected documents
        similarity_scores[name] = score

    # Add the selected documents to the top of the ranking
    ranked_documents = selected_documents + [doc for doc in retrieved_documents.keys() if doc not in selected_documents]

    # Calculate cosine similarity scores for the rest of the documents
    for name in ranked_documents:
        if name not in similarity_scores:
            document_vector = vectorizer.transform([retrieved_documents[name]])
            similarity_scores[name] = cosine_similarity(query_vector, document_vector)[0][0]
    
    # Prepare the final ranking with similarity scores
    ranked_results = [(name, similarity_scores[name]) for name in ranked_documents]
...
eel.start("main.html", size=(2000, 800))
